...
const PDFReader = forwardRef(({ currentPDF, currentPage, setCurrentPage, toggleFullScreen }, ref) => {
  ...
  const handleNextPage = () => {
    if (numPages && currentPage < numPages) {
      ...
      // First try to use the plugin's GoToNextPage if available
      if (window.pdfGoToNextPage) {
        window.pdfGoToNextPage();
      } else {
        // Otherwise update our state directly
        setCurrentPage(currentPage + 1);
        
        // Try using the viewer instance if available
        if (window.pdfViewer) {
          window.pdfViewer.jumpToPage(currentPage);
        }
      }
    }
  };
  ...
  const handlePreviousPage = () => {
    if (currentPage > 1) {
      ...
      // First try to use the plugin's GoToPreviousPage if available
      if (window.pdfGoToPreviousPage) {
        ...
        // Try using the viewer instance if available
        if (window.pdfViewer) {
          window.pdfViewer.jumpToPage(currentPage - 2);
        }
      }
    }
  };
